      ...
      <View style={styles.card}>
        <View style={styles.cardContent}>
          ...
          <View style={styles.cardBottom}>
            ...
            <TouchableOpacity style={styles.buyButtonWrapper} onPress={() => handleBuySeed(item)} disabled={money < item.price} activeOpacity={0.8}>
              <LinearGradient colors={money >= item.price ? ['#2E7D32', '#1B5E20'] : ['#37474F', '#263238']} style={styles.buyButton}>
                <ShoppingCart color={money >= item.price ? "#FFF" : "#78909C"} size={16} />
                <Text style={[styles.buyButtonText, money < item.price && styles.buyButtonTextDisabled]}>Acquire</Text>
              </LinearGradient>
            </TouchableOpacity>
          </View>
        </View>
    ...
    <View style={[styles.card, { marginLeft: 0, marginBottom: 16 }]}>
      <View style={[styles.cardContent, { paddingLeft: 16 }]}>
        ...
        <View style={styles.cardBottom}>
          ...
          <TouchableOpacity style={styles.buyButtonWrapper} onPress={() => handleBuyConsumable(item)} disabled={money < item.price} activeOpacity={0.8}>
            <LinearGradient colors={money >= item.price ? ['#0288D1', '#01579B'] : ['#37474F', '#263238']} style={styles.buyButton}>
              <ShoppingCart color={money >= item.price ? "#FFF" : "#78909C"} size={16} />
              <Text style={[styles.buyButtonText, money < item.price && styles.buyButtonTextDisabled]}>Acquire</Text>
            </LinearGradient>
          </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.tabs}>
        ...
        <TouchableOpacity style={[styles.tab, tab === 'supplies' && styles.activeTab]} onPress={() => setTab('supplies')}>
...
const styles = StyleSheet.create({
  ...
  rarityBadge: { borderWidth: 1, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 12, backgroundColor: 'rgba(0,0,0,0.3)' },
